...
Your training_img_filenames will be traversed in a loop until `count=len(training_img_filenames)` synthetic images are cloned to disk, with each synthetic clone having between `min_random_augmentations=3` and `max_random_augmentations=8` randomizations applied but only up to intensities seen that individually did not affect model output substantially.  If you want to risk exposing your model to training on more extreme datasets such as very bright etc., you can control generating sensible (`realism=0.5`) random augmentation ranges limited up to when small model differences start to show, reaching all the way up to values right at the edge (`realism=1`) of where the model starts to fail 100% of the time (ex. _exactly_ too bright for the model). `realism > 1` should be used sparingly as it may worsen realistic cases but _could_ force your model to generalize better.  `realism` near zero creates very little variation and is best avoided unless you really need very strict realism.  realism can also be set on a per-augmentation feature basis so you can control exactly how far to push each augmentation.
